...
git add .
git diff-index --quiet HEAD || git commit --message "[skip ci] Private ReaderBoard Update"
...
git add .
git diff-index --quiet HEAD || git commit --message "[skip ci] Public ReaderBoard Update"
git push origin --set-upstream main
rm -rf "$PUBLIC_DIR"
